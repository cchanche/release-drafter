...
const generateIndividualCommitsChangelog = (commits, config) => {
  ...
  if (commitsWithoutPullRequests.length > 0) {
    ...
    const writeCommit = (c) => `-${c.parsed?.scope ? ` **${c.parsed?.scope}:**` : ""}${` ${c.parsed?.subject || c.parsed?.header || c.message || "empty message"}`}${c.author?.user?.login ? ` @${c.author.user.login}` : ""}${c?.oid && repo && owner ? ` ([${c.oid.slice(
      0,
      7
...
export {
  generateIndividualCommitsChangelog
};
