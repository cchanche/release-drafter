...
const findReleases = async ({
  ...
  if (filterByRange && filterByRange !== '*') {
    core.debug(
      `Parsin raw range "${filterByRange}" as "${semver.validRange(
        filterByRange
      )}"`
    )
  }
...
const generateIndividualCommitsChangelog = (
  commitsWithoutPullRequests = [],
  config
) => {
  return commitsWithoutPullRequests.length > 0
    ? commitsWithoutPullRequests
        .map(
          (commit) =>
            `- ${commit.message}${
